<template>
  <div class="home" :class="{'bottom50':bottom50}" v-el:home-wrapper transition="fadeOutLeft">
    <scroll class="homeContent" v-ref:scroll :data="goods" :listenscroll="true">
      <div>
        <!-- 轮播广告 -->
...
//          });
      ...
      methods: {
        onScroll() {
          this.scrollTops = document.documentElement.scrollTop || window.pageYOffset || document.body.scrollTop;
          console.log(this.scrollTops);
          if(this.scrollTops > SCROLL_TOP){
            this.$dispatch('show.tab', true);
            this.bottom50 = true;
          }else if(this.scrollTops < SCROLL_TOP){
            this.$dispatch('show.tab', false);
            this.bottom50 = false;
          }
        },
